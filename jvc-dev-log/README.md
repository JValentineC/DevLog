...
I added a custom favicon with dark mode support. Two `.ico` files live in `public/`:

| File | Usage |
|------|-------|
| `public/favicon.ico` | Black icon — shown in **light mode** |
| `public/favicon-dark.ico` | White icon — shown in **dark mode** |

In `index.html`, both are linked with a `media` attribute so the browser picks the right one based on the user's color scheme:
